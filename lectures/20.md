---
title: "Методы редукции дисперсии. Адаптивные методы и Muon"
...
**Схема с возвращением** (sampling with replacement): индексы $i_1, \ldots, i_B$ выбираются i.i.d. равномерно из $\{1, \ldots, n\}$ (могут повторяться).
$$
...
Поскольку $i_j$ — i.i.d., слагаемые $\nabla f_{i_j}(x_k)$ — независимые одинаково распределённые случайные величины. Дисперсия среднего $B$ независимых величин:
$$
...
## Дисперсия мини-батч градиента: без возвращения
...
Слагаемые теперь **зависимы** (зная, что $i \in \mathcal{B}_k$, вероятность попадания остальных меняется). Из теории выборок без возвращения:
$$
\mathbb{E}\left[\left\|g^k - \nabla f(x_k)\right\|^2\right] = \frac{\sigma^2}{B} \cdot \frac{n - B}{n - 1}
$$
...
| $B \ll n$ | $\approx \dfrac{\sigma^2}{B}$ | $\approx \dfrac{\sigma^2}{B}$ |
...
**Идея (control variates)**: снижение дисперсии выборки $X$ с помощью другой случайной величины $Y$ с известным математическим ожиданием. Рассмотрим новую случайную величину $Z_\alpha$:
$$
...
## Ключевая идея редукции дисперсии — интуиция

**Интуиция:** Если $Y \approx X$, то $X - Y \approx 0$ — разность почти детерминирована, и дисперсия мала. При этом $\mathbb{E}[Y]$ нам известно точно, поэтому мы «якоримся» к точному значению и корректируем на маленькую стохастическую добавку.
...
* $\mathbb{E}[Y] = \frac{1}{n} \sum_{i=1}^n \nabla f_i(\tilde{x})$ — полный градиент в $\tilde{x}$, вычисляется один раз.
* Оценка градиента SVRG:
  $$
...
Обозначим $\bar{x}^{(k)} = \frac{1}{k} \sum_{l=0}^{k-1} x^{(l)}$ — среднее векторов $x^{(l)}$ после $k - 1$ шагов.
...
* Результат для усреднённого вектора $\bar{x}^{(k)}$, но справедлив и для лучшего $x^{(k)}_{best}$.
...
## SAG: неравномерное сэмплирование
...
## Эксперимент: VR методы vs SGD на логистической регрессии

![](exp_svrg_vs_sgd.pdf){width=95%}

**Вывод**: SGD (пост. шаг) застревает на noise floor. GD сходится линейно, но медленно при $\kappa \gg 1$. SAG и SVRG **линейно** сходятся ценой стохастической итерации, обгоняя GD по числу проходов.
...
$L(w) = w^2$, $\;\nabla L = 2w$. Обновление Adam (1D, без bias correction):

\vspace{-0.3em}

\begin{align*}
m_t &= \beta_1 m_{t-1} + (1-\beta_1)\cdot 2w_{t-1}\\
v_t &= \beta_2 v_{t-1} + (1-\beta_2)\cdot 4w_{t-1}^2\\
w_t &= w_{t-1} - \eta\,\frac{m_t}{\sqrt{v_t} + \varepsilon}
\end{align*}

. . .

**Ключевое наблюдение.** При $|w| \to 0$:

* $m_t \approx (1-\beta_1)\cdot 2w \;\propto\; w$
* $\sqrt{v_t} \approx \sqrt{(1-\beta_2)}\cdot 2|w| \;\propto\; |w|$

. . .

Отношение: $\dfrac{m_t}{\sqrt{v_t}} \approx \dfrac{w}{|w|} = \operatorname{sign}(w) = \pm 1$ — **не зависит** от $|w|$!

$\Rightarrow$ шаг $\approx \pm\eta$ при любом $|w|$ $\Rightarrow$ предельный цикл $w \in \{+\eta/2,\; -\eta/2\}$.

SGD: шаг $\propto w \to 0$ — сходится.
:::
::::
...
## Muon: вывод через наискорейший спуск в спектральной норме \footnote{\href{https://jeremybernste.in/writing/deriving-muon}{J. Bernstein "Deriving Muon". 2025.}} \footnote{\href{https://arxiv.org/pdf/2503.12645}{Kovalev, D. (2025). Understanding Gradient Orthogonalization.}}
...
* Коэффициенты эвристические: к $UV^\top$ точно не сходятся ($p(1)\approx0.70$), но за 5 шагов загоняют $\sigma_i$ в окрестность 1
:::
::::
