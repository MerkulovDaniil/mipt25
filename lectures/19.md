...
Если $\nabla f$ липшицево непрерывен, то имеем:
...
1. Разложение квадрата расстояния до минимума:
    $$
    \|x_{k+1}-x^{*}\|^{2} = \|x_k - \alpha \nabla f_{i_k}(x_k) - x^*\|^2 =\|x_k-x^{*}\|^{2}-2\alpha\langle\nabla f_{i_k}(x_k),x_k-x^{*}\rangle
    +\alpha^{2}\|\nabla f_{i_k}(x_k)\|^{2}.
    $$

2. Берём условное мат. ожидание по $i_k$, используем несмещённость, ограниченность дисперсии и выпуклость $f$ ($\langle \nabla f(x_k), x_k - x^* \rangle \ge f(x_k) - f^*$):
    $$
...
3. Переносим и берём полное мат. ожидание, суммируем (телескопируем) по $t=0,\dots,k-1$:
    $$
    \frac{1}{k}\sum_{t=0}^{k-1}\mathbb{E}\!\bigl[f(x_t)-f^{*}\bigr] \le \frac{\|x_0-x^{*}\|^{2}}{2\alpha k} + \frac{\alpha \sigma^{2}}{2}.
    $$

4. Выпуклость $f$ и неравенство Йенсена для $\bar x_k$ дают результат.
...
:::: {.columns}
::: {.column width="50%"}
![Линейная регрессия с МНК](lls_VR.svg)
:::

::: {.column width="50%"}
![Логистическая регрессия](logreg_VR.svg)
:::
::::
