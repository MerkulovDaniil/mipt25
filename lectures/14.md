...
:::: {.columns}
::: {.column width="40%"}
Запишем ([-@eq-err_decomposition2]) для некоторого фиксированного индекса $k$:
...
где $\|x\|^2_A = x^{\top}Ax$ и $\kappa(A) = \frac{\lambda_1(A)}{\lambda_n(A)}$ - это число обусловленности матрицы $A$, $\lambda_1(A) \geq ... \geq \lambda_n(A)$ - собственные значения матрицы $A$
...
* Так как $W$ невырожденна, существует единственное решение $Wx = b$. Решение методом сопряженных градиентов даёт довольно плохую сходимость.
* Во время работы CG ошибка растёт экспоненциально (!), пока внезапно не становится нулевой, когда находится единственное решение.
...
\beta_k = \frac{\nabla f(x_{k+1})^\top (\nabla f(x_{k+1}) - \nabla f(x_k))}{d_k^\top (\nabla f(x_{k+1}) - \nabla f(x_k))}.
$$
Однако данная формула — это формула Хестенса-Штифеля (Hestenes-Stiefel, HS). Для неквадратичного случая существует несколько различных вариантов выбора $\beta_k$, которые совпадают в квадратичном случае, но отличаются на общих функциях.

## Метод Флетчера-Ривза (Fletcher-Reeves, FR)

Метод Флетчера-Ривза (1964) — исторически первый нелинейный метод сопряженных градиентов.

**Идея:** Непосредственно перенести формулу $\beta_k$ из линейного (квадратичного) CG на общий случай. В квадратичном случае $r_k = -\nabla f(x_k)$, и формула $\beta_k = \dfrac{\langle r_{k+1}, r_{k+1} \rangle}{\langle r_{k}, r_{k} \rangle}$ принимает вид:
...
1. $d_0 = -\nabla f(x_0)$
2. $\alpha_k$ — из линейного поиска (например, условия Вульфа)
3. $x_{k+1} = x_k + \alpha_k d_k$
4. $\beta_k^{FR} = \dfrac{\|\nabla f(x_{k+1})\|^2}{\|\nabla f(x_k)\|^2}$
5. $d_{k+1} = -\nabla f(x_{k+1}) + \beta_k^{FR} d_k$

. . .

:::{.callout-caution}
Метод Флетчера-Ривза гарантированно сходится при использовании сильных условий Вульфа для линейного поиска. Однако на практике метод может «застревать» — генерировать малые шаги без существенного уменьшения функции, не имея механизма автоматического перезапуска.
:::

## Метод Полака-Рибьера (Polak-Ribière, PR)

Метод Полака-Рибьера (1969) модифицирует формулу для $\beta_k$, используя разность градиентов:

$$
\beta_k^{PR} = \frac{\nabla f(x_{k+1})^\top \left(\nabla f(x_{k+1}) - \nabla f(x_k)\right)}{\|\nabla f(x_k)\|^2}
$$

. . .

**Идея и преимущество:** В квадратичном случае $\beta_k^{PR} = \beta_k^{FR}$, так как невязки ортогональны: $\nabla f(x_{k+1})^\top \nabla f(x_k) = 0$. Однако для общих (неквадратичных) функций $\beta_k^{PR}$ может принимать отрицательные значения, что эквивалентно автоматическому перезапуску метода — направление спуска сбрасывается ближе к антиградиенту, когда последовательные градиенты становятся почти параллельны. Это делает метод PR значительно более устойчивым на практике по сравнению с FR.

. . .

:::{.callout-caution}
На практике часто используют модификацию $\beta_k^{PR+} = \max(\beta_k^{PR}, 0)$, которая гарантирует, что направление спуска всегда является направлением убывания, и обеспечивает глобальную сходимость при сильных условиях Вульфа.
:::

## Сравнение вариантов нелинейного CG

| Метод | $\beta_k$ | Особенности |
|---|---|---|
| Флетчер-Ривз (FR) | $\dfrac{\|\nabla f(x_{k+1})\|^2}{\|\nabla f(x_k)\|^2}$ | Гарантия сходимости, может застревать |
| Полак-Рибьер (PR) | $\dfrac{\nabla f(x_{k+1})^\top (\nabla f(x_{k+1}) - \nabla f(x_k))}{\|\nabla f(x_k)\|^2}$ | Встроенный рестарт, лучше на практике |
| Хестенс-Штифель (HS) | $\dfrac{\nabla f(x_{k+1})^\top (\nabla f(x_{k+1}) - \nabla f(x_k))}{d_k^\top (\nabla f(x_{k+1}) - \nabla f(x_k))}$ | Эквивалентен PR в квадратичном случае |

Все три формулы совпадают для квадратичных функций с точным линейным поиском. Для неквадратичных задач на практике рекомендуется использовать метод Полака-Рибьера с периодическим рестартом (обнуление $\beta_k$ каждые $n$ шагов или при $\beta_k < 0$).
...
## Сравнение FR, PR, HS. $\mu = 1$, без рестарта

$$
f(x) = \frac{\mu}{2} \|x\|_2^2 + \frac1m \sum_{i=1}^m \log (1 + \exp(- y_i \langle a_i, x \rangle)) \to \min_{x \in \mathbb{R}^n}
$$

![](cg_nl_fr_pr_hs_1000_300_1_None.pdf)

## Сравнение FR, PR, HS. $\mu = 10$, без рестарта

$$
f(x) = \frac{\mu}{2} \|x\|_2^2 + \frac1m \sum_{i=1}^m \log (1 + \exp(- y_i \langle a_i, x \rangle)) \to \min_{x \in \mathbb{R}^n}
$$

![](cg_nl_fr_pr_hs_1000_300_10_None.pdf)

## Сравнение FR, PR, HS. $\mu = 0.1$, без рестарта

$$
f(x) = \frac{\mu}{2} \|x\|_2^2 + \frac1m \sum_{i=1}^m \log (1 + \exp(- y_i \langle a_i, x \rangle)) \to \min_{x \in \mathbb{R}^n}
$$

![](cg_nl_fr_pr_hs_1000_300_0.1_None.pdf)

## Сравнение FR, PR, HS. $\mu = 1$, рестарт каждые 20 итераций

$$
f(x) = \frac{\mu}{2} \|x\|_2^2 + \frac1m \sum_{i=1}^m \log (1 + \exp(- y_i \langle a_i, x \rangle)) \to \min_{x \in \mathbb{R}^n}
$$

![](cg_nl_fr_pr_hs_1000_300_1_20.pdf)

## Сравнение FR, PR, HS. $\mu = 1$, рестарт каждые 50 итераций

$$
f(x) = \frac{\mu}{2} \|x\|_2^2 + \frac1m \sum_{i=1}^m \log (1 + \exp(- y_i \langle a_i, x \rangle)) \to \min_{x \in \mathbb{R}^n}
$$

![](cg_nl_fr_pr_hs_1000_300_1_50.pdf)
...
![](cg_nl_pr_vs_prplus_1000_300_1.pdf)