...
Пусть $f : S \to \mathbb{R}$ — функция, заданная на множестве $S$ в евклидовом пространстве $\mathbb{R}^n$. Если $x_0 \in \mathbf{ri }(S)$ и $f$ дифференцируема в точке $x_0$, то либо $\partial f(x_0) = \emptyset$, либо $\partial f(x_0) = \{\nabla f(x_0)\}$. Более того, если $f$ выпукла, то первый случай невозможен.
:::
...
1. Предположим, что $s \in \partial f(x_0)$ для некоторого $s \in \mathbb{R}^n$, отличного от $\nabla f(x_0)$. Пусть $v \in  \mathbb{R}^n$ — единичный вектор. Поскольку $x_0$ — внутренняя точка $S$, существует $\delta > 0$ такое, что $x_0 + tv \in S$ 
:::

. . .

::: {.column width="50%"}
для всех $0 < t < \delta$. По определению субградиента имеем
 $$
...
2. Отсюда $\langle s - \nabla f(x_0), v \rangle \geq 0$. В силу произвольности $v$ можно положить
 $$
    v = -\frac{s - \nabla f(x_0)}{\| s - \nabla f(x_0) \|},
    $$ 
 что приводит к $s = \nabla f(x_0)$.
3. Кроме того, если функция $f$ выпукла, то по дифференциальному условию выпуклости $f(x) \geq f(x_0) + \langle \nabla f(x_0), x - x_0 \rangle$ для всех $x \in S$. Но по определению это означает, что $\nabla f(x_0) \in \partial f(x_0)$.
:::
::::
