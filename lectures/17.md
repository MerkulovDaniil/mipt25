...
![Субградиентный метод для SVM. Сравнение различных стратегий выбора шага](svm_hinge_subgrad.pdf)

## Метод опорных векторов. Немонотонность

![Немонотонность $f(\theta_k)$ при постоянном шаге. Убывающий шаг уменьшает осцилляции. Сходимость по аргументу зависит от стратегии](svm_nonmonotone.pdf)
...
![Фактическая сходимость $f_k^{\text{best}} - f^*$ и теоретическая верхняя оценка для выпуклого и сильно выпуклого случаев. Наклоны совпадают: $\mathcal{O}(1/\sqrt{k})$ и $\mathcal{O}(1/k)$](convergence_rate_theory_vs_practice.pdf)
...
![Шаг Поляка существенно превосходит стандартные стратегии и обеспечивает сходимость по аргументу. Однако требует знания $f^*$](subgrad_polyak_step.pdf)
...
![С ростом размерности $n$ субградиентный метод сходится медленнее. На лог-лог графике все кривые параллельны прямой $1/\sqrt{k}$](subgrad_dimension_effect.pdf)
