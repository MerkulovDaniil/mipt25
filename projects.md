...
## Выбор темы
🗓 15 апреля 2026. 🦄 4 балла

Выбрать команду для проекта. Размер команды - 2-4 человек.
...
* В недавнем [блогпосте](https://dao-lab.ai/blog/2026/gram-newton-schulz/) авторы предлагают оптимизацию алгоритма Newton–Schulz через переход к Gram-матрицам, позволяющую существенно ускорить ортогонализацию без потери качества. В рамках проекта предлагается сравнить классический Newton–Schulz и Gram Newton–Schulz с точки зрения точности и численной стабильности, а также экспериментально оценить ускорение обучения на задаче (например, [nanoGPT](https://github.com/karpathy/nanoGPT)), включая влияние на сходимость оптимизатора Muon.
* В статье [SOAP: Improving and Stabilizing Shampoo using Adam](https://arxiv.org/abs/2409.11321) авторы предлагают новый оптимизатор второго порядка, который комбинирует преимущества Shampoo (предобуславливание через блочные матрицы) и стабильность Adam, что позволяет ускорить обучение больших моделей без потери качества. В рамках проекта предлагается реализовать упрощённую диагональную версию SOAP (или AdaFisher) и сравнить её с Adam на задачах fine-tuning малых языковых моделей (например, на GLUE-бенчмарке), оценив компромисс между скоростью сходимости, потреблением памяти и финальной точностью.
* В статье [Friendly Sharpness-Aware Minimization](https://arxiv.org/abs/2403.12350) авторы показывают, что можно улучшить обобщающую способность моделей, модифицируя классический SAM: удаление детерминированной компоненты градиента и использование стохастического шума позволяет достичь того же качества за один проход вместо двух. В рамках проекта предлагается сравнить оригинальный SAM, Friendly-SAM и базовый SGD/Adam на задачах с ограниченным бюджетом эпох (например, обучение ResNet-18 на CIFAR-10 или fine-tuning DistilBERT на небольшом датасете), проанализировав влияние методов на «остроту» минимума и устойчивость к переобучению.
* В статье [LoRA-Pro: Are Low-Rank Adapters Properly Optimized?](https://arxiv.org/abs/2407.18242) авторы демонстрируют, что стандартное обновление низкоранговых матриц в LoRA не эквивалентно градиентному шагу при полном fine-tuning, и предлагают корректировать градиенты адаптеров для восстановления этой эквивалентности. В рамках проекта предлагается реализовать LoRA-Pro и сравнить его с базовым LoRA на задачах параметр-эффективного дообучения (классификация текста, генерация ответов), исследовав, насколько метод сокращает разрыв в качестве между адаптерами и полным обновлением весо
